...
export const disableScroll = (value) => {
  ...
  function disableScroll() {
    if ($body)
      $body.addEventListener("onMouseUp", preventDefault, { passive: false });
  }
  function enableScroll() {
    if ($body)
      $body.removeEventListener("onMouseUp", preventDefault, {
        passive: false,
      });
  }
};
...
export const menuMobile = () => {
    ...
    .forEach(function (itemUl) {
      ...
      if (obj > 0) {
        ...
        itemUl.insertAdjacentHTML(
          "afterbegin",
          `<div class="link__openSubmenu"></div>`
        );
      }
    });
  Array.prototype.slice
    .call(objMenu.querySelectorAll("nav > ul > li"))
    .forEach(function (itemUl) {
  ...
  function checkActiveMenu() {
    removeParent();
    Array.prototype.slice
      .call(objMenu.querySelectorAll(".is-active"))
      .forEach(function (itemActive) {
  ...
  objMenu.addEventListener("click", function (e) {
    checkActiveMenu();
    if (e.target && e.target.classList.contains("link__openSubmenu"))
...
/*
	Сart Handle Pay - Modified for Static Site
*/
export const handlePay = async (cartItems) => {
  // For static sites, redirect to contact page or show contact info
  alert(
    "For purchases, please contact us directly at: your-email@example.com or call 0405 133 761"
  );

  // Alternative: redirect to contact page
  // window.location.href = '/contact-us/';

  // Alternative: Use external payment processor like PayPal, Square, etc.
  // Example with PayPal:
  // window.location.href = 'https://www.paypal.com/paypalme/youraccount';
};
