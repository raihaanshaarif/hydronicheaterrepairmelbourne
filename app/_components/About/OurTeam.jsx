    ...
    <>
      <div className="section__indent">
        <div className="container container__fluid-lg">
          ...
          />
          <div
            className={`${styles.direction__wrapper} grid md:grid-flow-col md:grid-cols-2 gap-[30px]`}
          >
            {dataTeamDirection.map(
              ({
                id,
                title,
                subtitle,
                ...
                <div key={id} className={`${styles.direction} relative grid`}>
                  ...
                  <div className={`${styles.direction__content}`}>
                    {title ? (
                      <h6
                        className={`${styles.direction__title} ttsubtitle_sm`}
                      >
                        {title}
                      </h6>
                    ) : null}
            ...
            >
              {dataTeamRersonal.map(({ id, title, subtitle, img__path }) => (
                <SwiperSlide key={id} className="text-center">
                  <div
                    className={`${styles.personal} inline-block text-center`}
                  >
                    {img__path ? (
                      <div
                        className={`${styles.personal__img} relative obj__indent-centerBefore`}
                      >
                        <Image
                          src={img__path}
                          width={195}
                    ...
                    {title ? (
                      <div className={styles.personal__title}>{title}</div>
                    ) : null}
                    {subtitle ? (
